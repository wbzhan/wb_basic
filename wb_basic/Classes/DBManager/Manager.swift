...
public extension CKDBManager {
    ...
    func deleteDB(_ whereDic: [String: Any]?, completed: Completed?) {
        var dic = whereDic
        if whereDic == nil {
            let properties = self.propertyKeyValues()
            if let key = Self.ck_primarykey() {
                dic = [key: properties.filter({ $0.name == key }).first?.value as Any]
            }else {
                dic = [:]
                for property in properties {
                    dic![property.name] = property.value
                }
            }
    ...
    func propertyKeyValues() -> [PropertyModel] {
        ...
        return (mirror.children).map { (key, value) in
            PropertyModel.init(name: key, value: value)
        }.filter{ property in
...
//            .filter({ $0 != nil })
    ...
    static func selectModel(_ whereDic: [String: Any]?, order: String?, page: Int, rows: Int) -> [Any] {
        let rs = DBManager.standard.selectDB(whereDic, type: self, order: order, page: page, rows: rows)
        let array = rs.map({ (obj) -> Self? in
            if let dic = obj as? [String: Any] {
                return self.setPropetyValue(dic)
            }
    ...
    ///批量删除
    ///whereDic 数据筛选条件key(字段名)，value(字段值), nil则清空当前表所有数据
    static func deleteDB<T: CKDBManager>(_ array: [T], completed: Completed?) {
        var index = 0
        for item in array {
            index = index + 1
            item.deleteDB(nil) {
